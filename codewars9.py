...
class Solution:
    ...
    def figure_mapper_double(self, i, j, num, count):
        ...
        # 1) we check if we encounter a new number, add it to the dictionary of currently connected vertices,
        #        if the new number in fake_vertices, we add current ones there
        if i >= 1 and self.list_shape[i - 1][j] not in ['|', '+', '-', ' '] and self.list_shape[i - 1][j] not in self.double_vertices[num]:
            for x in self.double_vertices:
                if self.list_shape[i - 1][j] in self.double_vertices[x]:
        ...
        '''vertical down'''
        if i < self.depth - 1 and self.list_shape[i + 1][j] not in ['|', '+', '-', ' '] and self.list_shape[i + 1][j] not in self.double_vertices[num]:
            for x in self.double_vertices:
                if self.list_shape[i + 1][j] in self.double_vertices[x]:
                    self.double_vertices[num] += self.double_vertices[x]
                    if x in self.fake_vertices:
        ...
        '''horizontal left'''
        if j >= 1 and self.list_shape[i][j - 1] not in ['|', '+', '-', ' '] and self.list_shape[i][j - 1] not in self.double_vertices[num]:
            for x in self.double_vertices:
        ...
        '''horizontal right'''
        if j < self.width - 1 and self.list_shape[i][j + 1] not in ['|', '+', '-', ' '] and self.list_shape[i][j + 1] not in self.double_vertices[num]:
            for x in self.double_vertices:
                if self.list_shape[i][j + 1] in self.double_vertices[x]:
                    self.double_vertices[num] += self.double_vertices[x]
                    if x in self.fake_vertices:
